...
// mongoose
//   .connect("mongodb://127.0.0.1/cfDB", {
...
app.get(
  ...
  (req, res) => {
    //authentication code goes between URL and callback
    ...
    Movies.findOne({ "Director.Name": req.params.directorName })
      .then((movie) => {
        if (!movie) {
          return res
            .status(404)
            .send("Error: director " + req.params.directorName + " not found.");
        } else {
    ...
    Movies.findOne({ "Genre.Title": req.params.genreName })
      .then((movie) => {
        if (!movie) {
          return res
            .status(404)
            .send("Error: genre " + req.params.Genre + " not found.");
        } else {
  ...
  (req, res) => {
    Users.findOneAndUpdate(
      ...
      .then((data) => {
        ...
        res
          .status(200)
          .send(req.params.MovieID + " was removed from favorites");
      })
      .catch((err) => {
        console.error(err);
        res.status(500).send("Error: " + err);
